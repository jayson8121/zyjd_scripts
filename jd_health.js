            ...
            if (taskId === -1) {
              ...
              await doTask(data?.data?.result?.taskVos[0].shoppingActivityVos[0]?.taskToken, 22, 0);//完成任务
               ...
               for (let i = vo.times; i < vo.maxTimes; ++i) {
				   if(vo.taskType === 1){
						console.log(`去完成${vo.followShopVo[i]?.shopName}任务`);
				   }else{
                ...
                }else if(vo.taskType === 25){
					 console.log(`请手动完成${vo.taskName}任务`)
				}
              }  
            }
          }
...
function shareCodesFormat() {
  return new Promise(async resolve => {
	...
	AuthorShareCode =  await getAuthorShareCode('https://raw.githubusercontent.com/FearNoManButGod/jd_scripts/main/AuthorShareCode/jd_health.json');
