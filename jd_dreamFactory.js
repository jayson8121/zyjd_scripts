...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN(), res2 = await updateTuanIdsCDN("https://raw.githubusercontent.com/FearNoManButGod/jd_scripts/main/AuthorShareCode/empty.json")
  if (!res) res = await updateTuanIdsCDN('https://raw.githubusercontent.com/FearNoManButGod/jd_scripts/main/AuthorShareCode/empty.json');
...
function shareCodesFormat() {
  return new Promise(async resolve => {
	...
	AuthorShareCode =  await getAuthorShareCode('https://raw.githubusercontent.com/FearNoManButGod/jd_scripts/main/AuthorShareCode/jd_dreamFactory.json');
    $.newShareCodes = [...(AuthorShareCode || [])];
  ...
  return new Promise(async resolve => {
    ...
    if (!tuanActiveId) {
      ...
      if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
        ...
        if (!$.tuanConfigs) {
          await updateTuanIdsCDN('https://raw.githubusercontent.com/FearNoManButGod/jd_scripts/main/AuthorShareCode/empty.json');
          if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
            tuanActiveId = $.tuanConfigs['tuanActiveId'];
            console.log(`拼团活动ID: 获取成功 ${tuanActiveId}\n`)
          } else {
